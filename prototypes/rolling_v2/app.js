...
const game = {
    ...
    farmersOnTiles: {},
    caughtFarmers: {},
    farmerTrolleyTiles: {},  // { tileIndex: farmerIndexAtSourceTile } — track which trolley belongs to which farmer
    runCount: 0,             // how many runs in this session (scales trolley probability)
...
/** Spawns a tractor (farmer obstacle) on the given tile index with trolleys behind it. */
function buildFarmerAt(scene, physIdx) {
    ...
    const trolleyMat = makeFlatMat(scene, 'trolleyMat_' + physIdx, 0.55, 0.55, 0.55);

    // Tractor body is offset right of track and faces toward the player (front at negative Z).
    const body = BABYLON.MeshBuilder.CreateBox('tBody', { width: 1.2, height: 0.8, depth: 1.8 }, scene);
    body.position.set(2.5, 0.5, 0); body.parent = root; body.material = tMat;

    // Rear cabin sits behind the front grill so facing is visually obvious.
    const cab = BABYLON.MeshBuilder.CreateBox('tCab', { width: 0.8, height: 0.9, depth: 0.8 }, scene);
    cab.position.set(0, 0.85, 0.35); cab.parent = body; cab.material = tMat;

    const hood = BABYLON.MeshBuilder.CreateBox('tHood', { width: 0.75, height: 0.45, depth: 0.75 }, scene);
    hood.position.set(0, 0.25, -0.55); hood.parent = body; hood.material = tMat;

    const grillMat = makeFlatMat(scene, 'grillMat_' + physIdx, 0.95, 0.9, 0.75);
    const grill = BABYLON.MeshBuilder.CreateBox('tGrill', { width: 0.62, height: 0.3, depth: 0.1 }, scene);
    grill.position.set(0, 0.28, -0.98); grill.parent = body; grill.material = grillMat;

    const exhaust = BABYLON.MeshBuilder.CreateCylinder('tExhaust', { diameter: 0.12, height: 0.55, tessellation: 6 }, scene);
    exhaust.position.set(-0.28, 0.85, 0.25); exhaust.parent = body; exhaust.material = wMat;

    // Rear wheels are larger and sit at the back (+Z).
    [[-0.6, 0.45], [0.6, 0.45]].forEach(([x, z]) => {
        const w = BABYLON.MeshBuilder.CreateCylinder('wRear', { diameter: 0.86, height: 0.3 }, scene);
        w.rotation.z = Math.PI / 2; w.position.set(x, -0.12, z); w.parent = body; w.material = wMat;
    });
    // Front wheels are smaller and sit at the front (-Z).
    [[-0.5, -0.58], [0.5, -0.58]].forEach(([x, z]) => {
        const w = BABYLON.MeshBuilder.CreateCylinder('wFront', { diameter: 0.45, height: 0.2 }, scene);
        w.rotation.z = Math.PI / 2; w.position.set(x, -0.3, z); w.parent = body; w.material = wMat;
    });

    // Generate trolley count: scales with run count, fewer in early runs
    const trolleyChance = Math.min(0.9, 0.3 + game.runCount * 0.2);
    let trolleyCount = 0;
    if (Math.random() < trolleyChance) {
        if (game.runCount < 2) {
            trolleyCount = Math.random() < 0.6 ? 0 : (Math.random() < 0.7 ? 1 : 2);
        } else {
            trolleyCount = Math.floor(Math.random() * 4); // 0-3
        }
    }

    // Create trolley blocks behind the tractor (away from player) along forward tiles.
    const trolleyMeshes = [];
    for (let t = 0; t < trolleyCount; t++) {
        const trolleyTileIdx = (physIdx + t + 1) % BOARD_SIZE;
        
        // Stop if there's another farmer blocking the trolley chain
        if (game.farmersOnTiles[trolleyTileIdx]) {
            break;
        }
        
        const trolleyPos = getTileWorldPosition(trolleyTileIdx);
        const trolleyRot = getTileRotation(trolleyTileIdx);

        // Create trolley with same size as tractor body
        const trolleyBox = BABYLON.MeshBuilder.CreateBox('trolley_' + physIdx + '_' + t, {
            width: 1.2, height: 0.8, depth: 1.6
        }, scene);
        
        // Align trolley to the right of the track like the tractor body.
        // Offset is (2.5, 0.5, 0) in local space, rotated by tile rotation.
        const offsetLocal = new BABYLON.Vector3(2.5, 0.5, 0);
        const rotMatrix = BABYLON.Matrix.RotationX(trolleyRot.x);
        const offsetWorld = BABYLON.Vector3.TransformCoordinates(offsetLocal, rotMatrix);
        const finalPos = trolleyPos.add(offsetWorld);
        
        trolleyBox.position.copyFrom(finalPos);
        trolleyBox.rotation.x = trolleyRot.x;
        trolleyBox.material = trolleyMat;
        
        trolleyMeshes.push(trolleyBox);
        game.farmerTrolleyTiles[trolleyTileIdx] = physIdx;
    }

    game.farmersOnTiles[physIdx] = { root, trolleyCount, trolleyMeshes };
}

/** Checks if a tile is blocked by a farmer or farmer trolley. */
function isFarmerBlockedTile(tileIdx) {
    return !!game.farmersOnTiles[tileIdx] || !!game.farmerTrolleyTiles[tileIdx];
}

/** Removes a farmer obstacle and all its trolleys from the board. */
function removeFarmerAt(physIdx) {
    const data = game.farmersOnTiles[physIdx];
    if (!data) return;
    
    // Dispose farmer meshes
    ...
    // Dispose trolley meshes
    if (data.trolleyMeshes) {
        data.trolleyMeshes.forEach(mesh => {
    ...
    // Remove trolley tile references for this farmer
    Object.keys(game.farmerTrolleyTiles).forEach(key => {
        if (game.farmerTrolleyTiles[key] === physIdx) {
            delete game.farmerTrolleyTiles[key];
        }
    });
}
...
function doRoll() {
    ...
    movePlayer(positions, rotations, () => {
        ...
        const frontActive = !game.flockMode || game.frontGroup > 0;
        const rearActive = game.flockMode && game.rearGroup > 0;
        const frontLandedOnFarmer = frontActive && isFarmerBlockedTile(physIdx);
        const rearLandedOnFarmer = rearActive && isFarmerBlockedTile(rearPhysIdx);
        const landedFarmerIndices = [];
        // Map trolley tiles to their source farmer
        if (frontLandedOnFarmer) {
            const farmerIdx = game.farmerTrolleyTiles[physIdx] || physIdx;
            landedFarmerIndices.push(farmerIdx);
        }
        if (rearLandedOnFarmer && !landedFarmerIndices.includes(rearPhysIdx)) {
            const farmerIdx = game.farmerTrolleyTiles[rearPhysIdx] || rearPhysIdx;
            if (!landedFarmerIndices.includes(farmerIdx)) landedFarmerIndices.push(farmerIdx);
        }
        ...
        // Reverse farmer logic: getting caught happens on direct landing.
        if (frontLandedOnFarmer) registerCaughtFarmer(physIdx, 'front');
        if (rearLandedOnFarmer) registerCaughtFarmer(rearPhysIdx, 'rear');

        const clearByBehind = updateFarmerEscapeState(physIdx, rearPhysIdx);
        const clearByPass = clearFarmersByPassing(physIdx, rearPhysIdx, frontActive, rearActive, landedFarmerIndices);
        rewardFarmerClear(clearByBehind + clearByPass);

        const finalizeCaughtEncounter = () => {
            const clearByLanding = clearFarmersByIndices(landedFarmerIndices);
            rewardFarmerClear(clearByLanding);
            finishTurn();
        };

        if (frontLandedOnFarmer || rearLandedOnFarmer) {
            if (game.flockMode) {
                showFlockFarmerEncounter(finalizeCaughtEncounter);
            } else {
                showFarmerEncounter(finalizeCaughtEncounter);
            }
            return;
        }
...
function rewardFarmerClear(count) {
    if (count <= 0) return;
    const clearBonus = BALANCE.COIN_TRACTOR_CLEAR * count;
    game.cash += clearBonus;
    awardMeals(2 * count);
    showFeedback('🍎 Escaped the farmer! +🪙' + clearBonus + ' +🍎' + (2 * count));
}

function clearFarmersByIndices(indices) {
    let cleared = 0;
    indices.forEach(idx => {
        if (!game.farmersOnTiles[idx]) return;
        removeFarmerAt(idx);
        delete game.caughtFarmers[idx];
        cleared++;
    });
    return cleared;
}

function clearFarmersByPassing(frontPhysIdx, rearPhysIdx, frontActive, rearActive, landedFarmerIndices) {
    const candidates = [];

    if (frontActive) {
        const passedFront = (frontPhysIdx - 1 + BOARD_SIZE) % BOARD_SIZE;
        if (!landedFarmerIndices.includes(passedFront)) candidates.push(passedFront);
    }
    if (rearActive) {
        const passedRear = (rearPhysIdx - 1 + BOARD_SIZE) % BOARD_SIZE;
        if (!landedFarmerIndices.includes(passedRear)) candidates.push(passedRear);
    }

    // Farmers currently in the caught-state flow must be cleared only by that flow.
    const eligible = [...new Set(candidates)].filter(idx => !game.caughtFarmers[idx]);
    return clearFarmersByIndices(eligible);
}

function registerCaughtFarmer(farmerIdx, sourceGroup) {
    // If this is a trolley tile, get the actual farmer it belongs to
    const actualFarmerIdx = game.farmerTrolleyTiles[farmerIdx] || farmerIdx;
    if (!game.farmersOnTiles[actualFarmerIdx]) return;

    if (!game.caughtFarmers[actualFarmerIdx]) {
        game.caughtFarmers[actualFarmerIdx] = {
            frontEscaped: !game.flockMode,
            rearEscaped: !game.flockMode,
            soloEscaped: false,
        };
    }

    if (!game.flockMode) return;
    if (sourceGroup === 'front') game.caughtFarmers[actualFarmerIdx].frontEscaped = false;
    if (sourceGroup === 'rear') game.caughtFarmers[actualFarmerIdx].rearEscaped = false;
}

function updateFarmerEscapeState(frontPhysIdx, rearPhysIdx) {
    let cleared = 0;
    const frontActive = !game.flockMode || game.frontGroup > 0;
    const rearActive = game.flockMode && game.rearGroup > 0;

    Object.keys(game.caughtFarmers).forEach(key => {
        const farmerIdx = Number(key);
        const afterFarmer = (farmerIdx + 1) % BOARD_SIZE;
        const state = game.caughtFarmers[farmerIdx];

        if (!game.flockMode) {
            if (frontPhysIdx === afterFarmer) state.soloEscaped = true;
            if (state.soloEscaped) {
                removeFarmerAt(farmerIdx);
                delete game.caughtFarmers[farmerIdx];
                cleared++;
            }
            return;
        }

        if (!frontActive) state.frontEscaped = true;
        if (!rearActive) state.rearEscaped = true;
        if (frontPhysIdx === afterFarmer) state.frontEscaped = true;
        if (rearActive && rearPhysIdx === afterFarmer) state.rearEscaped = true;

        if (state.frontEscaped && state.rearEscaped) {
            removeFarmerAt(farmerIdx);
            delete game.caughtFarmers[farmerIdx];
            cleared++;
        }
...
function handleTileLanding(physIdx) {
    ...
    switch (def.type) {
        ...
        case 'danger':
            ...
            break;
        case 'farmer':
            showFeedback('🚜 Farmer tile! Find a safe way behind it.');
            break;
        case 'pit':
...
/** Shows the farmer encounter overlay for group animals (sacrifice a member option). */
function showFlockFarmerEncounter(onResolved) {
    const div = $('farmerChoices');
    div.innerHTML = '';

    const total = game.frontGroup + game.rearGroup;
    const memberEmoji = game.animalDef ? game.animalDef.emoji : '🐾';
    addChoiceButton(div, memberEmoji + ' Sacrifice a Member (' + total + ' left)', total > 1, () => {
        sacrificeRandomGroupMember();
        closeOverlay('farmerOverlay');
        showFeedback(memberEmoji + ' Left a member behind as bait! Reach behind the farmer with both groups.');
        onResolved();
    });
    ...
    addChoiceButton(div, '🎲 Run for it! (4-6 success)', true, () => {
        ...
        setTimeout(() => {
            if (r >= 4) {
                showEventPopup('🏃', 'SUCCESS!', 'The group outran the farmer!', [
                    { label: 'CONTINUE', action: onResolved }
                ]);
            } else {
                sacrificeRandomGroupMember();
                const remaining = game.frontGroup + game.rearGroup;
                if (remaining <= 0) {
                    showEventPopup('🚜', 'ALL CAUGHT!', 'The farmer caught all your group members!', [
                        { label: 'BACK TO SANCTUARY', action: () => { game.cash = 0; returnToSanctuary(); } }
                    ]);
                } else {
                    showEventPopup('🚜', 'CAUGHT!', remaining + ' members remaining.', [
                        { label: 'CONTINUE', action: onResolved }
                    ]);
                }
            }
...
/** Removes one random member from either the front or rear group. */
function sacrificeRandomGroupMember() {
    const candidates = [];
    if (game.frontGroup > 0) candidates.push('front');
    if (game.rearGroup > 0) candidates.push('rear');
    if (candidates.length === 0) return;

    const group = candidates[Math.floor(Math.random() * candidates.length)];
    if (group === 'front') {
        game.frontGroup--;
        const mesh = game.frontGroupMeshes.pop();
        if (mesh) { mesh.getChildMeshes().forEach(m => m.dispose()); mesh.dispose(); }
    } else {
        game.rearGroup--;
        const mesh = game.rearGroupMeshes.pop();
        if (mesh) { mesh.getChildMeshes().forEach(m => m.dispose()); mesh.dispose(); }
    }
    updateRibbon();
}
...
function updateUI() {
    ...
    const farmerCount = Object.keys(game.farmersOnTiles).length;
    const farmerDebug = $('farmerDebug');
    if (farmerDebug) farmerDebug.textContent = '🚜 ' + farmerCount;
}
